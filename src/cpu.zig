...
pub fn mem_read(cpu: *const types.CPU, addr: u16) u8 {
    return bus.mem_read(&cpu.bus, addr);
}

pub fn mem_write(cpu: *types.CPU, addr: u16, data: u8) void {
    return bus.mem_write(&cpu.bus, addr, data);
}

pub fn mem_read_u16(cpu: *const types.CPU, pos: u16) u16 {
    return bus.mem_read_u16(&cpu.bus, pos);
}

pub fn mem_write_u16(cpu: *types.CPU, pos: u16, data: u16) void {
    return bus.mem_write_u16(&cpu.bus, pos, data);
}
...
pub fn step(cpu: *types.CPU) bool {
    ...
    switch (info.instruction) {
        ...
        .rra => {
            ...
            const sum = @as(u16, cpu.accumulator) + value + @intFromBool(cpu.status.carry);
            const result: u8 = @truncate(sum);
            cpu.status.carry = sum > 0xff;
            cpu.status.overflow = (value ^ result) & (result ^ cpu.accumulator) & 0x80 != 0;
            cpu.accumulator = result;
            update_zero_and_negative_flags(cpu, cpu.accumulator);
        },
    }
